...
$.initProv = function(prov, city) {
    var provEl = $(prov);
    var cityEl = $(city);
    var defaultProv = arguments[2]?arguments[2]:'山东省';
    var defaultCity = arguments[3]?arguments[3]:'济南市';
...
// 筛选
$.initProvSerach = function(prov, city) {
    var provEl = $(prov);
    var cityEl = $(city);
    var defaultProv = arguments[2]?arguments[2]:'不限';
    var defaultCity = arguments[3]?arguments[3]:'不限';
    var hasDefaultProv = (typeof(defaultCity) != 'undefined');

    var provHtml = '';
    //  provHtml += '<option value="-1">请选择</option>';
    for(var i = 0; i < $._cityInfoSerach.length; i++) {
        provHtml += '<option value="' + i + '"' + ((hasDefaultProv && $._cityInfoSerach[i].n == defaultProv) ? ' selected="selected"' : '') + '>' + $._cityInfoSerach[i].n + '</option>';
    }
    provEl.html(provHtml);
    $.initCitiesSerach(provEl, cityEl, defaultCity);
    provEl.change(function() {
        $.initCitiesSerach(provEl, cityEl);
    });
};

$.initCitiesSerach = function(provEl, cityEl, defaultCity) {
    var hasDefaultCity = (typeof(defaultCity) != 'undefined');
    if(provEl.val() != '' && parseInt(provEl.val()) >= 0) {
        var cities = $._cityInfoSerach[parseInt(provEl.val())].c;
        var cityHtml = '';
