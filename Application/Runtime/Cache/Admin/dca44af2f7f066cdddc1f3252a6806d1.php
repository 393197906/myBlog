					...
					<tbody>
                        <?php $_GET['color'] = array('fff','#eee','#ddd','#','#ccc','#ccc','#ccc'); function DG_printList($arr,$colorId=0){ $count = count($arr); for($i =0;$i!=$count;$i++){ echo "<tr style=\"background-color:".$_GET['color'][$colorId]."\">"; echo "<td>".$arr[$i]['id']."</td>"; echo "<td>".$arr[$i]['cname']."</td>"; echo "<td>".$arr[$i]['ename']."</td>"; if(!($arr[$i]['pid'])){ echo "<td>最高节点</td>"; }else{ echo "<td>".$arr[$i]['pi']." ".($colorId+1)."级节点</td>"; } echo "<td>
                                    ...
                                    <button class=\"button danger delete\" >删除</button>
                                     </td>"; echo "</tr>"; if(is_array($arr[$i]['zi'])){ $newColorId = $colorId+1; DG_printList($arr[$i]['zi'],$newColorId); } } } DG_printList($subjectList); ?>
					</tbody>
				</table>
